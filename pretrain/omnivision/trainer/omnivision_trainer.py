...
class OmnivisionTrainer(object):
    ...
    def load_checkpoint(self):
        ...
        ckpt_path = get_resume_checkpoint(self.checkpoint_conf.resume)
    ...
    def run(self):
        ...
        if self.mode == "train":
            ...
            self.run_train()
        logging.info(f"Total time: {datetime.timedelta(seconds=int(time.time() - start))}")
    ...
    def run_train(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            logging.info(f"Train: total time: {datetime.timedelta(seconds=int(time.time() - start))}")
            start = time.time()
            # Run val
            if self.epoch % self.val_epoch_freq == 0:
                self.run_val()
            logging.info(f"Val: total time: {datetime.timedelta(seconds=int(time.time() - start))}")
            start = time.time()
            self.epoch += 1
    ...
    def train_epoch(self, train_loader):
        ...
        opt_names = ['max_lr','min_lr','weight_decay']
        opt_mts = OrderedDict(
            [(name, AverageMeter(name, self.device, ":.2e")) for name in opt_names]
        )
        progress = ProgressMeter(
            iters_per_epoch,
            [batch_time, data_time, mem, *opt_mts.values(), *metrics_mts.values(), *loss_mts.values()],
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            # TODO: 计算训练完当前的epoch还需要多久
            import datetime
            remain_time = (iters_per_epoch - data_iter) * batch_time.avg
            remain_time = datetime.timedelta(seconds=int(remain_time))
            # 修改progressor, 添加一个 eta 的属性
            progress.eta = str(remain_time)
            # TODO: 添加 打印 lr
            min_lr = 10.
            max_lr = 0.
            ...
            opt_mts['max_lr'].update(max_lr)
            opt_mts['min_lr'].update(min_lr)
            weight_decay_value = None
            for group in self.optim.optimizer.param_groups:
                if group["weight_decay"] > 0:
                    weight_decay_value = group["weight_decay"]
            opt_mts['weight_decay'].update(weight_decay_value)        
            
            mem.update(torch.cuda.max_memory_allocated() // 1e9)

            if data_iter % self.logging_conf.log_freq == 0 and self.distributed_rank == 0:
                progress.display(data_iter)

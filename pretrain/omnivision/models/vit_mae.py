...
class Decoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, final_dim)
    ...
    def build_pos_embedding(
        ...
        if share_pos_embed is True:
            ...
            self.pos_embed = nn.Parameter(
                ...
                torch.zeros(1, np.prod(patches_layout) + \
                            first_patch_idx, embed_dim)
            )
    ...
    def forward(
        ...
        if self.decoder_pred is not None:
            interim = [self.decoder_pred(el) for el in interim]
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if learnable_pos_embed:
            self.pos_embed = nn.Parameter(
                torch.zeros(1, total_num_patches, embed_dim))
        else:
            self.register_buffer(
                "pos_embed", get_sinusoid_encoding_table(
                    total_num_patches, embed_dim)
            )
        ...
        self.pre_logits = nn.Linear(embed_dim, num_classes)  if num_classes > 0 else nn.Identity()
        ...
        if self.masked_image_modeling:
            ...
            self.mask_token = nn.Parameter(
                torch.zeros(1, mask_token_embed_dim))
        ...
        if self.classifier_feature == "cls_token" and (
            ...
            if isinstance(x, list):
                decoder_patch_features = [
                    el[:, self.first_patch_idx:] for el in x]
            else:
    ...
    def get_intermediate_features(
        ...
        for name in names:
            if name.startswith("blkCLS"):
                ...
                output.append(
                    interms[-1][:, self.first_patch_idx:, ...].mean(dim=1))
        return output
...
class VisionTransformerPretrain(nn.Module):
    """
    Vision transformer Pretrain. Adding stochastic depth makes it a DeiT.
    """
    ...
    def __init__(
        ...
        ) if self.weight_contrast > 0 else None
    ...
    def forward_decoder(self, x, orig_input_shape, mask=None, use_checkpoint=False):
        # x = self.norm(x)
        if self.post_encoder:
            x_dtype = x.dtype
            x = self.post_encoder(x).to(x_dtype)
    ...
    def info_nce_loss(self, z, temperature):
        ...
        sim = torch.exp(torch.einsum(
            "if, jf -> ij", z, gathered_z) / temperature)
    ...
    def forward(
        ...
        assert (not self.masked_image_modeling) or (mask is not None)
        t1, t2 = x.shape[1], mask.shape[1]
        x1, x2 = x[:,:t1//2], x[:,t1//2:]
        mask1, mask2 = mask[:, :t2//2], mask[:, t2//2:]
        
        z1 = self.forward_features(
            x1, npatch_to_keep, mask=mask1, use_checkpoint=use_checkpoint)
        u1 = self.projector(torch.mean(
            z1[:, self.first_patch_idx:, :], dim=1))  # Skip cls token
        x1_pred = self.forward_decoder(
            z1, x1.shape, mask1, use_checkpoint=use_checkpoint)
        with torch.no_grad():
            z2 = self.forward_features(
                x2, npatch_to_keep, mask=mask2, use_checkpoint=use_checkpoint)
            u2 = self.projector(torch.mean(z2[:, self.first_patch_idx:, :], dim=1))  # Skip cls token
            # x2_pred = self.forward_decoder(
            #     z2, x2.shape, mask2, use_checkpoint=use_checkpoint)
        
        # pred = torch.stack([x1_pred, x2_pred], dim=1)
        
        # Contrastive loss
        loss_contrast = self.info_nce_loss(
            torch.cat([u1, u2]), temperature=self.temperature) * self.weight_contrast

        return x1_pred, loss_contrast
